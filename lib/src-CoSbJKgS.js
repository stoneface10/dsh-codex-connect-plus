...
*/
function createOpenAICodexAdapter(auth, resolveAttachments, resolveModelMaxRetries = () => 0) {
...
const CODEX_CONNECT_VERSION = "0.1.0-beta.5";
//#endregion
...
const MAX_OPENAI_CODEX_MODEL_RETRIES = 2;
...
const Config = z.object({
	modelMaxRetries: z.number().step(1).min(0).max(2).default(0),
...
function apply(ctx, config) {
	...
	const adapter = createOpenAICodexAdapter(auth, () => ctx.get("attachments"), () => resolveOpenAICodexSettings(current()).modelMaxRetries);
