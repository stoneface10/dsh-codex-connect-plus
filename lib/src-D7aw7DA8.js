...
import { createUserMessage, resolveImageAttachmentAccess, resolveRetryPolicy } from "@deepseek-ai/dsh-llm";
...
*/
const OPENAI_CODEX_RETRYABLE_CODES = Object.freeze([
	"EMPTY_RESPONSE",
	"RATE_LIMIT",
	"SERVER",
	"TIMEOUT",
...
function createOpenAICodexProfile(provider, maxRetries) {
	return {
...
*/
function createOpenAICodexAdapter(auth, resolveAttachments, resolveModelMaxRetries = () => 0, resolveProcessPath = () => void 0) {
	const provider = auth.provider;
	let cachedMaxRetries;
	let cachedProfiles;
	const profiles = () => {
		const maxRetries = resolveModelMaxRetries();
		if (cachedProfiles !== void 0 && cachedMaxRetries === maxRetries) return cachedProfiles;
		cachedMaxRetries = maxRetries;
		cachedProfiles = /* @__PURE__ */ new Map([[OPENAI_CODEX_PROVIDER, createOpenAICodexProfile(provider, maxRetries)]]);
	...
	return new PiAiAdapter({
		...
		resolveAttachments,
		resolveImageAccess: (attachments, ref) => resolveImageAttachmentAccess(attachments, resolveProcessPath, ref)
	});
}
//#endregion
...
const CODEX_CONNECT_VERSION = "0.1.0-beta.6";
//#endregion
...
const DEFAULT_OPENAI_CODEX_MODEL_MAX_RETRIES = 0;
/** Upper bound exposed by this plugin for deliberate bounded recovery. */
const MAX_OPENAI_CODEX_MODEL_RETRIES = 10;
...
function decodeOpenAICodexSettings(value) {
	...
	if (typeof modelMaxRetries !== "number" || !Number.isInteger(modelMaxRetries) || modelMaxRetries < 0 || modelMaxRetries > 10) return void 0;
...
const Config = z.object({
	modelMaxRetries: z.number().step(1).min(0).max(10).default(0),
...
function apply(ctx, config) {
	...
	const adapter = createOpenAICodexAdapter(auth, () => ctx.get("attachments"), () => resolveOpenAICodexSettings(current()).modelMaxRetries, (hostPath) => ctx.get("fs")?.processPathFromHostPath(hostPath));
...
//#endregion
export { decodeCodexImageResponse as A, readOpenAICodexRateLimits as B, recordOpenAICodexSearchRequest as C, CODEX_IMAGE_GENERATE_URL as D, CODEX_IMAGE_EDIT_URL as E, assertNoOpenAICodexProviderConflict as F, OPENAI_CODEX_PROVIDER as G, logoutOpenAICodex as H, diagnoseOpenAICodex as I, OpenAICodexCredentialStore as K, openAICodexConflictMessage as L, resolveCodexImageSize as M, safeCodexImageHttpError as N, CODEX_IMAGE_MODEL as O, VIEW_IMAGE_TOOL_NAME as P, OPENAI_CODEX_USAGE_URL as R, installOpenAICodexSearchEvent as S, CODEX_IMAGE_GENERATE_TOOL_NAME as T, openAICodexAuthStatus as U, loginOpenAICodex as V, OPENAI_CODEX_AUTH_FILENAME as W, MAX_OPENAI_CODEX_MODEL_RETRIES as _, name as a, resolveOpenAICodexSettings as b, OPENAI_CODEX_SEARCH_URL as c, DEFAULT_OPENAI_CODEX_MODEL_MAX_RETRIES as d, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as f, DEFAULT_OPENAI_CODEX_SETTINGS as g, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as h, inject as i, detectCodexImageType as j, createCodexImageRequest as k, OpenAICodexSearchProvider as l, DEFAULT_OPENAI_CODEX_SEARCH_MODE as m, OPENAI_CODEX_SETTINGS_NS as n, OPENAI_CODEX_BASE_URL as o, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as p, openAICodexAuthPath as q, apply as r, OPENAI_CODEX_SEARCH_PROVIDER as s, Config as t, mapOpenAICodexSearchResponse as u, OPENAI_CODEX_SETTINGS_NAMESPACE as v, CODEX_IMAGE_EDIT_TOOL_NAME as w, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as x, decodeOpenAICodexSettings as y, parseOpenAICodexUsage as z };
