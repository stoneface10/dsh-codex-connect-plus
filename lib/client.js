...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const RETRY_OPTIONS = Array.from({ length: 11 }, (_, modelMaxRetries) => modelMaxRetries);
		const CONFIG_FIELDS = [
			"modelMaxRetries",
			"enableSearch",
			"enableImageTool",
		...
		function OpenAICodexConfiguration({ snapshot, saveConfig, t }) {
			...
			const validRetries = draft !== void 0 && Number.isInteger(draft.modelMaxRetries) && draft.modelMaxRetries >= 0 && draft.modelMaxRetries <= 10;
			...
			const save = async () => {
				if (draft === void 0 || !snapshot.writable || !valid) return;
				const desired = {
					...draft,
				...
				setFeedback("idle");
				try {
					const accepted = (await saveConfig(desired, snapshot.revision)).value;
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("select", {
										...
										children: RETRY_OPTIONS.map((modelMaxRetries) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
											value: modelMaxRetries,
											children: modelMaxRetries === 0 ? t("retryNone") : modelMaxRetries === 1 ? t("retryOnce") : t("retryCount", { count: modelMaxRetries })
										}, modelMaxRetries))
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										style: bodyStyle$1,
		...
		const UNAVAILABLE_SETTINGS_SNAPSHOT = {
			status: "unavailable",
			value: void 0,
			base: void 0,
			user: void 0,
		...
		const UNAVAILABLE_SETTINGS_SAVE = () => Promise.resolve(UNAVAILABLE_SETTINGS_SNAPSHOT);
		const pageStyle = {
			display: "flex",
			flexDirection: "column",
			gap: 18,
		...
		function OpenAICodexSettings({ t, configSnapshot, saveConfig, embedded = false }) {
		...
		function OpenAICodexPluginCard({ t, useOpenAICodexSettings, saveConfig }) {
			const configSnapshot = useOpenAICodexSettings((snapshot) => snapshot);
		...
		const en = {
			...
			modelMaxRetriesHelp: "Retries use 1–30 second exponential backoff and include temporarily unclassified pi-ai failures such as Codex WebSocket errors and server overload. Each retry repeats the full model request and may consume additional subscription capacity.",
			retryNone: "0 — do not retry",
			retryOnce: "1 — retry once",
			retryCount: "{count} — retry up to {count} times",
			...
			invalidModelRetries: "Automatic retries must be a whole number from 0 through 10.",
		...
		const zh = {
			...
			modelMaxRetriesHelp: "重试采用 1–30 秒指数退避，并覆盖 Codex WebSocket 错误和服务过载等暂未正确分类的 pi-ai 故障。每次重试都会重新发送完整模型请求，可能额外消耗订阅额度。",
			retryNone: "0 — 不重试",
			retryOnce: "1 — 重试一次",
			retryCount: "{count} — 最多重试 {count} 次",
			...
			invalidModelRetries: "模型自动重试次数必须是 0 至 10 的整数。",
		...
		const SETTINGS_LOCALE = "settings.openai-codex";
		...
		function apply(ctx) {
			ctx.effect(() => ctx.locale.register(SETTINGS_LOCALE, {
				zh,
				en
			...
			const cardInjected = () => ({
				...
				async saveConfig(desired, expectedRevision) {
					await configScope.mutate([
						{
							op: "set",
							path: ["modelMaxRetries"],
							value: desired.modelMaxRetries
						},
						{
							op: "set",
							path: ["enableSearch"],
							value: desired.enableSearch
						},
						{
							op: "set",
							path: ["enableImageTool"],
							value: desired.enableImageTool
						},
						{
							op: "set",
							path: ["enableImageGeneration"],
							value: desired.enableImageGeneration
						},
						{
							op: "set",
							path: ["searchModel"],
							value: desired.searchModel
						},
						{
							op: "set",
							path: ["searchMode"],
							value: desired.searchMode
						},
						{
							op: "set",
							path: ["searchContextSize"],
							value: desired.searchContextSize
						},
						{
							op: "set",
							path: ["searchMaxOutputTokens"],
							value: desired.searchMaxOutputTokens
						}
					], expectedRevision);
			...
			for (const key of [CODEX_IMAGE_GENERATE_TOOL_NAME, CODEX_IMAGE_EDIT_TOOL_NAME]) ctx.slots.inject("tool.call.toolview", () => ctx.slots.register({
				...
				key,
				inject: (sessionId) => ({ loadImage: createCodexImageLoader(ctx.sessions, sessionId) })
			}, CodexImageToolView));
		}
		//#endregion
		exports.apply = apply;
