...
const CODEX_CONNECT_VERSION = "0.1.0-alpha.1";
//#endregion
...
function safeCodexImageHttpError(status, body) {
	...
	detail = detail.replace(/\beyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\b/gu, "[redacted token]").replace(/("?(?:access|refresh|token|authorization)"?\s*[:=]\s*")([^"\s]+)/giu, "$1[redacted]").replace(/Bearer\s+[A-Za-z0-9._~+/=-]+/giu, "Bearer [redacted]").replace(/data:image\/[a-z0-9.+-]+;base64,[A-Za-z0-9+/=]+/giu, "[redacted image data]");
