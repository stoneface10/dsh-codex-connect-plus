...
插件继承 DSH 普通 retry policy。Beta.6 将可选上限扩展到十次并采用 1–30 秒指数退避，同时把当前 pi-ai 错误压平后留下的 `PI_AI_ERROR` 临时纳入有界重试，以避免 Codex WebSocket 与服务过载等瞬时故障直接终止长任务。
...
用户可在插件配置中显式选择 0 至 10 的任意整数。零仍只发起一次请求；十表示首次请求失败后最多再发起十次完整请求。该设置以长任务可靠性换取潜在订阅额度消耗，不承诺任何服务端额度比例。
...
- 普通短任务保持零重试；无法人工守候的长任务可设为 10，并关注完整请求重放带来的订阅额度消耗；
